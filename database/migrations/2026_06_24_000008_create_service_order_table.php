...
class CreateServiceOrderTable extends Migration
{
    public function up()
    {
        Schema::create('service_order', function (Blueprint $table) {
            ...
            $table->integer('vehicules_customer_id');

            $table->primary(['id', 'users_id', 'users_role_id', 'vehicules_id', 'vehicules_customer_id'], 'service_order_pk');
            ...
            $table->index(['vehicules_id', 'vehicules_customer_id'], 'fk_service_order_vehicules1_idx');

            $table->foreign(['users_id', 'users_role_id'], 'fk_service_order_users1')
                ->references(['id', 'role_id'])
                ->on('users');

            $table->foreign(['vehicules_id', 'vehicules_customer_id'], 'fk_service_order_vehicules1')
